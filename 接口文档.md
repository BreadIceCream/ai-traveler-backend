...
| List<Pois> searchPoiFromExternalApi(String city, String keywords); | **功能:** 从第三方API（高德/谷歌）获取POI。 **思路:** 调用API从第三方地图服务搜索POI，默认返回5个结果 |
| List<Pois> searchPoiFromDb(String city, String keywords); | 功能：从数据库中获取POI。 |
| void asyncEmbeddingAndSave(List<Pois> poisList); | 功能：事务方法，异步处理embedding向量并将pois写入数据库 |
