...
| 方法 | 功能描述 |
| :---- | :---- |
| Pois getPoiById(UUID poiId) | **功能:** 根据ID获取POI详细信息 |
| List\<Pois\> searchPoiFromDb(String city, String keywords) | **功能:** 从数据库中根据城市和关键词搜索POI |
| List\<Pois\> searchPoiFromExternalApiAndSave(String city, String keywords, Integer searchNumber, Function\<JSONArray, List\<String\>\> generateDescriptions) | **功能:** 使用第三方API（高德/谷歌）搜索POI，支持自定义描述生成函数，并保存至数据库 |
| List\<Pois\> searchPoiFromExternalApiAndSave(String city, String keywords) | **功能:** 使用第三方API搜索POI并保存至数据库，默认使用LLM生成POI描述 |
| void asyncEmbeddingAndSaveUpdate(List\<Pois\> poisList) | **功能:** 异步处理POI的embedding向量生成并保存或更新到数据库 |
| List\<Pois\> semanticSearchPois(String queryText, String city, int topK) | **功能:** 基于语义向量相似度搜索POI，用于灵感激发模式（F-3.5.1），可作为tool供大模型调用 |
...
| 方法 | 功能描述 |
| :---- | :---- |
| List\<Pois\> getPoisItems(UUID userId, UUID conversationId, Integer version) | **功能:** 获取会话中推荐的POI列表 |
| List\<NonPoiItem\> getNonPoiItems(UUID userId, UUID conversationId, Integer version) | **功能:** 获取会话中推荐的非POI项（如活动、节日等）列表 |
| boolean addPois(UUID userId, UUID conversationId, List\<UUID\> poiIds) | **功能:** 将POI添加到会话推荐项中 |
| boolean addNonPoiItems(UUID userId, UUID conversationId, List\<UUID\> nonPoiItemIds) | **功能:** 将非POI项添加到会话推荐项中 |
| boolean removePoisFromItems(UUID userId, UUID conversationId, List\<UUID\> poiIds) | **功能:** 从会话推荐项中移除POI（不删除POI本身） |
| boolean removeNonPoiFromItems(UUID userId, UUID conversationId, List\<UUID\> nonPoiItemIds) | **功能:** 从会话推荐项中移除非POI项（级联删除非POI项本身） |
...
说明: 该服务继承自MyBatis-Plus的IService，提供基础的CRUD操作。当前无额外自定义方法。
...
| 方法 | 功能描述 |
| :---- | :---- |
| WebSearchResults webSearch(UUID conversationId, WebSearchParam param) | **功能:** 使用第三方搜索API搜索网页，返回搜索结果（包含原始查询、结果数量和网页列表） |
| ExtractResult extractItemsFromWebPageAndSave(UUID userId, String city, UUID webPageId) | **功能:** 从指定网页中提取旅游相关信息（POI和非POI项），并保存到数据库 |
