...
点击 [这里](https://github.com/BreadIceCream/ai-traveler-frontend) 查看前端项目效果图。
...
4. **Redisson 分布式锁**：
   *   **分层锁机制**：实现了 Trip(旅程) -> Day(日程) -> Item(事项) 三层粒度的并发控制。
   *   **读写锁 (ReadWriteLock)**：父级（如 Trip）持有读锁时，允许子级（如 Day）修改；父级持有写锁时，独占整个分支。
   *   **联锁 (MultiLock)**：原子性一次获取整条链路的锁（如修改 Item 时，自动锁定 Trip.Read + Day.Read + Item.Lock），防止死锁。
   *   **看门狗 (Watchdog)**：自动续期，防止业务长耗时导致锁意外释放。
