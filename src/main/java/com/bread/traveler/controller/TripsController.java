...
public class TripsController {
    ...
    public Result updateTripInfo(@RequestParam UUID userId, @RequestBody TripDto dto) {
        Trips trip = tripsService.updateTripInfo(userId, dto);
        return trip == null ? Result.serverError("更新失败") : Result.success("更新成功", trip);
    }

    @GetMapping("/list")
    @Operation(summary = "获取用户所有行程")
    public Result getAllTripsOfUser(@RequestParam UUID userId) {
        List<Trips> trips = tripsService.getAllTripsOfUser(userId);
        return trips.isEmpty() ? Result.success("获取成功，暂无行程", trips) : Result.success("获取成功", trips);
    }

    @GetMapping("/detail")
    @Operation(summary = "获取完整行程信息")
    ...
    @Operation(summary = "删除行程，级联删除所有日程和item")
